...
def _find_toc_like_page_indices(pages: list[_PageLines], known_titles: set[str]) -> set[int]:
	...
	while index < len(pages):
		...
		end_index = index
		while end_index + 1 < len(pages) and (strong_flags[end_index + 1] or weak_flags[end_index + 1]):
			end_index += 1
...
def parse_pdf(path: Path, source_id: str) -> list[ParsedRecord]:
	...
	if toc_drop_indices:
		...
		logger.info("PDF parse TOC/index pages dropped file=%s pages=%s", source_path, dropped_toc_pages)
	...
	if content_start is None:
		logger.info("PDF parse content start not found file=%s; using first non-TOC page", source_path)
		start_page_index = 0
		start_line_index = 0
	else:
	...
	if non_toc_pages and start_page_index > 0:
		front_matter_pages_before_start = [
			page.page for page in non_toc_pages[:start_page_index]
		]
		logger.info(
